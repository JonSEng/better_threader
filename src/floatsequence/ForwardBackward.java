		...
		public void incrementExpectedCounts(float[][] alphas, float[][] betas, int d, boolean viterbi) {
			...
			for (int s=0; s<lattice.numStates(d, 0); ++s) {
				if (viterbi) {
					...
					this.sequenceLogMarginalProbs[d] = SloppyMath.logAdd(sequenceLogMarginalProbs[d], betas[0][s]);
				}
			}
				...
				for (int prevS=0; prevS<numStates; ++prevS) {
					...
					for (int i=0; i<nextStates.length; ++i) {
						...
						if (viterbi) {
							...
							currentAlphas[nextState] = SloppyMath.logAdd(currentAlphas[nextState], prevAlpha + edgeLogPotential);
						}
					}
				}
